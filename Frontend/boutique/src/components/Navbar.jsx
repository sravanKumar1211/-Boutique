...
import { Search, ShoppingCart, Menu, Close, PersonAdd } from '@mui/icons-material';
...
function Navbar() {
  ...
  // Use Redux state ONLY. This is the "Single Source of Truth".
  ...
  return (
    <nav className="bg-[#5A0E24] text-white sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="text-xl font-bold tracking-wide px-2 py-1 rounded hover:ring-1 hover:ring-[#67B2D8]">
        ...
        <ul className="hidden md:flex items-center gap-6 text-xs font-semibold uppercase">
          {[{ to: '/', label: 'Home' }, { to: '/products', label: 'Shop' }, { to: '/about', label: 'About' }, { to: '/contact', label: 'Contact' }].map((item) => (
            <li key={item.label}>
              <Link to={item.to} className="px-2 py-1 rounded hover:ring-1 hover:ring-[#67B2D8]">{item.label}</Link>
            </li>
          ))}
        </ul>

        <div className="flex items-center gap-4">
          <form onSubmit={handleSearch} className="hidden lg:flex items-center bg-white rounded overflow-hidden h-10 w-[420px]">
            <input
              type="text"
          ...
          <Link to="/cart" className="relative flex items-center px-2 py-1 rounded hover:ring-1 hover:ring-[#67B2D8]">
            <ShoppingCart />
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden px-2 py-1 rounded hover:ring-1 hover:ring-[#67B2D8]">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[#76153C] border-t border-[#BF124D]">
          <ul className="flex flex-col gap-4 p-4 text-sm uppercase">
            ...
            <div className="flex items-center px-2 py-1 rounded hover:ring-1 hover:ring-[#67B2D8]">
            {isAuthenticated ? (
              <UserDashboard user={user} />
            ) : (
              <Link to="/login" className="flex items-center gap-1 text-xs">
            ...
            <form onSubmit={handleSearch} className="flex bg-white rounded overflow-hidden h-10">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <button className="bg-[#67B2D8] px-4"><Search className="text-black" /></button>
            </form>
          </ul>
        </div>
      )}
